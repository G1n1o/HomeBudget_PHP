...
if (isset($_POST['price'])) {
    ...
    $income = $db->prepare('INSERT INTO incomes VALUES (NULL, :user_ID, :category, :amount, :date, :comment)');
    $income->bindValue(':user_ID', $_SESSION['logged_id'], PDO::PARAM_INT);
    $income->bindValue(':category', $category, PDO::PARAM_INT);
    $income->bindValue(':amount', $amount, PDO::PARAM_STR);
    $income->bindValue(':date', $date, PDO::PARAM_STR);
    $income->bindValue(':comment', $comment, PDO::PARAM_STR);
    $income->execute();

    $accepted= 'Dodano przychód';

}
 
$incomesQuery = $db->query("SELECT id, name  FROM incomes_category_assigned_to_users WHERE user_id = {$_SESSION['logged_id']}");
